...
F1_ROWS = [
    ...
    ["This work", "drug–indication pair", "trial-agnostic approval forecast"],
]

# Figure 1B — dataset construction pipeline (left -> right).
F1_PIPELINE = [
    "AACT / ClinicalTrials.gov",
    "drug–condition tuples",
    "drug–indication programs",
    "FDA-label approval adjudication",
    "SMILES + ICD-10 + target / pathway / ADMET features",
    "temporal split",
]
...
F2_MODELS = [
    ("ChemAP-style", "drug-level approval", 0.462, 0.434, 0.488, 0.218, 0.199, 0.240, True),
    ("HINT adapted", "trial outcome", 0.619, 0.596, 0.643, 0.334, 0.307, 0.369, True),
    ("molecule only", "task-specific ablation", 0.721, 0.698, 0.741, 0.419, 0.381, 0.458, False),
    ("molecule + disease", "simple task-specific", 0.744, 0.721, 0.765, 0.489, 0.450, 0.528, False),
    ("full model", "task-specific full", 0.759, 0.738, 0.779, 0.512, 0.470, 0.553, False),
]
...
def figure1() -> None:
    ...
    for i, stage in enumerate(F1_PIPELINE):
        ...
        axB.text(x0 + bw / 2, yb + bh / 2, textwrap.fill(stage, width=18),
                 ha="center", va="center", fontsize=8.4)
        if i < n - 1:
            axB.annotate("", xy=(x0 + bw + gap, yb + bh / 2), xytext=(x0 + bw, yb + bh / 2),
                         arrowprops=dict(arrowstyle="-|>", color=C_HEADER, lw=1.4))

    # --- C: final cohort summary cards ---
    axC.axis("off")
    _panel_tag(axC, "C", "Final cohort summary (locked)")
    axC.set_xlim(0, 1)
    axC.set_ylim(0, 1)
    cards = [
        ...
        (f"{COHORT['smiles_coverage']:.2%}", f"SMILES coverage ({COHORT['smiles_dropped']} dropped)"),
        (f"{COHORT['icd_dropped']:,}", "dropped: missing ICD-10"),
    ]
    ncol, gx, gy = 4, 0.015, 0.06
...
def figure2() -> None:
    ...
    fig.suptitle("Figure 2 — Task-specific drug–indication model vs transferred baselines",
                 fontsize=14, fontweight="bold", y=1.02)

    labels = [f"{m[0]}\n({m[1]})" for m in F2_MODELS]
    ...
    handles = [Line2D([0], [0], marker="o", color=C_TRANSFER, lw=0, ms=8,
                      label="transferred from adjacent task"),
               Line2D([0], [0], marker="o", color=C_TASK, lw=0, ms=8,
                      label="task-specific (this work)")]
    axR.legend(handles=handles, loc="lower left", fontsize=8.5, frameon=False)
